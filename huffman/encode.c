...
void traverse_tree(const int depth, const Node *np)
{
  ...
  if (np->left == NULL){
    code[depth] = '\0';
    codeindex[np->symbol] = (char *)malloc(sizeof(char) * SIZE);
    strcpy(codeindex[np->symbol],code);   
    return;
  }
  if (np->left) { 
    code[depth] = '0';
...
int encode(const char *filename)
{
  ...
  for(i = 0; i< NSYMBOLS; i++){
    ...
    for(i = 0; i <NSYMBOLS; i++){
      if(tmp == i){
	fwrite(&codeindex[i],sizeof(codeindex[i]),1,nfp);
	break;
      }
    }
 }
